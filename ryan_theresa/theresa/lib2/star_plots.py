...
def emaps(star, eigeny, outdir, proj='ortho'):
    ...
    for j in range(ncurves):
        ...
        print(f"The {j}st/th eigen map's flux is {star.map.flux(theta=thet).eval()}")
...
def lightcurves(t, lcs, outdir, plotname):
    nharm, nt = lcs.shape
...
def eigencurves(t, lcs, outdir, eigencurves, ncurves=None):
    ...
    for i in range(ncurves):
        ...
        # print(lcs[i])
        ax.plot(t, lcs[i], label="E-curve {}".format(i+1))
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(outdir, f'{eigencurves}.png'))
    plt.close(fig)
