...
def map2d(cfile):
    ...
    # print(f"The flux y00 is {fit.pflux_y00} with shape {fit.pflux_y00.shape}")
    # print(f"The sys flux is {system.flux(fit.t, total=False)}")

    # print(f"The fit.t is {fit.t} with shape {fit.t.shape}")


    # return None

    print("Calculating planet and star fluxes at each filter.")
    ...
    print(f"It has taken {middle_time - start_time} seconds to get to the loop of the function.")
    ...
    for i in range(len(fit.wlmid)):
        ...
        print(f"It has taken {pca_time - middle_time} seconds to run the PCA.")
        ...
        np.savetxt(f'planet_eigeny_{m.lmax}.txt', m.eigeny, fmt='%d')
        plots.emaps(planet, m.eigeny, outdir, proj='rect')


        end_time = time.time()

        print(f"It has taken {end_time - pca_time} seconds to create the plot.")

        print(f"It has taken {end_time - start_time} seconds to run the whole code.")
