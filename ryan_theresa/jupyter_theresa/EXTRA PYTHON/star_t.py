...
def map2d(cfile):
    ...
    star, planet, system = star_utils.initsystem(fit, 1)
    ...
    fit.minvislon, fit.maxvislon = utils.vislon(planet, fit) #needs to be planet
    ...
    for i in range(len(fit.wlmid)):
        ...
        star, planet, system = star_utils.initsystem(fit, m.lmax)

        print("Running PCA to determine eigencurves.")
        m.eigeny, m.evalues, m.evectors, m.ecurves, m.lcs = \
        star_eigen.mkcurves(system, fit.t, m.lmax, fit.pflux_y00,
        ...
        np.savetxt(f'eigeny_{m.lmax}.txt', m.eigeny, fmt='%d')
